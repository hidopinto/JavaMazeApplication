...
/**
 * This Window is the main window in the app.
 * It displays the board, and all of the other widgets,
 * It runs the mini game(of 1v1),
 * and more...
 * MainWindow extends BasicWindow
 * MainWindow implements View
...
public class MainWindow extends BasicWindow implements View{
	...
	/**
	 * MazeWindow Constructor setting the Window's title as String, it's width and height as ints, it's board as CommonGameBoard, and it's displayer(md) as CommonDisplayer.
	 * @param title
	 * @param width
	 * @param height
	 * @param board
	 ...
	 * @see CommonDisplayer
	 */
	public MainWindow(String title, int width, int height,CommonGameBoard board, CommonDisplayer md, Display display) {
		super(title, width, height, display);
		this.board=board;
		this.md=md;
	}
	/**
	 * sets the MainWindow's board.
	...
	/**
	 * sets the MainWindow's md.
	 * @param md - a CommonDisplayer
	 */
	 ...
	 */
	/**
	 * this function defines all the widgets in the Window, and adds al the Listeners.
	 */
	@Override
	void initWidgets() {
		class flag
		...
		l1.setText("name of the maze");
				
		Text mazeName = new Text(shell, SWT.BORDER);
		mazeName.setLayoutData(new GridData(SWT.LEFT, SWT.CENTER, false, false, 1, 1));
		
		Label l2=new Label(shell, SWT.NONE);
		l2.setLayoutData(new GridData(SWT.LEFT, SWT.TOP, false, false, 1, 1) );
		l2.setText("number of columns");
				
		Text mazeCols = new Text(shell, SWT.BORDER);
		mazeCols.setLayoutData(new GridData(SWT.LEFT, SWT.CENTER, false, false, 1, 1));
		
		Label l3=new Label(shell, SWT.NONE);
		l3.setLayoutData(new GridData(SWT.LEFT, SWT.TOP, false, false, 1, 1) );
		l3.setText("number of rows");
				
		Text mazeRows = new Text(shell, SWT.BORDER);
		mazeRows.setLayoutData(new GridData(SWT.LEFT, SWT.CENTER, false, false, 1, 1));
		...
		generateMaze.addSelectionListener(new SelectionListener() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				l2.setVisible(false);	
				l3.setVisible(false);
				mazeName.setVisible(false);	
				mazeCols.setVisible(false);	
				mazeRows.setVisible(false);	
				setChanged();
				notifyObservers("GenerateMaze"+","+mazeName.getText()+":"+mazeCols.getText()+":"+mazeRows.getText());
				solved = false;		
			}
		...
		solveMaze.setText("solve The Maze!");
		solveMaze.setLayoutData(new GridData(SWT.LEFT, SWT.TOP, false, false, 1, 1));
		solveMaze.addSelectionListener(new SelectionListener() {
			...
			public void widgetSelected(SelectionEvent e) {
				if(solved==true)
					return;
				setChanged();
				notifyObservers("SolveMaze"+","+mazeName.getText());
				setChanged();
				notifyObservers("DisplaySolution"+","+mazeName.getText());
				solved = true;
			}
		...
		sound.setLayoutData(new GridData(SWT.LEFT, SWT.BOTTOM, false, false, 1, 1));
		sound.setImage(new Image(board.getDisplay(), new ImageData("animations/play.png")));
		
		board.setLayoutData(new GridData(SWT.FILL, SWT.FILL, false, false, 2, 2));
	...
	/**
	 * this function checks if a character has arrived to it's targetI and targetJ - won.
	...
	/**
	 * this function makes a character shoot a ball
	 * @param index - the character's index.
	 * @see Ball
	 */
	...
	/**
	 * displays the maze through the view
	 * @param m
	 */
	@Override
	public void displayMaze(Maze m) {
		//this.notifyObservers("GenerateMaze"+","+"maze"+":"+5+":"+5);
		//md.setBoard(board);
		md.setData(m);
		board.setLayoutData(new GridData(SWT.FILL, SWT.FILL, true,true,3,2));
		...
		if(board.gameCharecters.size()==0)
			board.gameCharecters.add(new ImgGameCharacter(0,0,((md.getRows()-1)) , ((md.getCols()-1)),null));
...
//		shell.setLayout(new GridLayout(2, false));
	...
	/**
	 * displays the solution through the view
	 * @param s
	 */
	@Override
